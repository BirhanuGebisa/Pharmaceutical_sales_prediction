...
def insert_to_tweet_table(dbName: str, df: pd.DataFrame, table_name: str) -> None:
    ...
    for _, row in df.iterrows():
        ...
        sqlQuery = f"""INSERT INTO {table_name} (user_id, promo2, customers, sales)
             VALUES(%s, %s, %s, %s);"""
...
if __name__ == "__main__":
    createDB(dbName='pharama_db')
    emojiDB(dbName='pharama_db')
    createTables(dbName='pharama_db')
    df = pd.read_csv('../data/score_table.csv')
    df.info()
    insert_to_tweet_table(dbName='pharama_db', df=df,

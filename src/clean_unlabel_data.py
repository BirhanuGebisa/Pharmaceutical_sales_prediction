...
app_logger = Application_Logger("../logs/unlabel_data.log").get_app_logger()

class Cleanunlabel:
    def __init__(self, df: pd.DataFrame, deep=False):
        """
            Returns a Cleanunlabel Object with the passed DataFrame Data set as its own DataFrame
            Parameters
            ----------
            df:
                Type: pd.DataFrame
            ...
            None
        """
        self.logger = Application_Logger("../logs/unlabel_data.log").get_app_logger()
        if(deep):
